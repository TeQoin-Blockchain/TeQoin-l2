...
import { saveBatch, markBatchSubmitted } from '../database/models';
...
export class BatchSubmitterService {
  ...
  private isSubmitting: boolean = false; // Submission lock
  ...
  private async submitBatch(): Promise<void> {
    ...
    if (this.isSubmitting) {
      logService('BATCH-SUBMITTER', 'Submission already in progress, skipping...');
      return;
    }
    ...
    try {
      ...
      const { stateRoot, transactionsRoot } = await this.collectBlockData(
        lastSubmitted + 1n,
        nextRotationBlock
      );
      ...
      const tx = await retryWithDefaults(async () => {
        return await this.contract!.submitBatch(
          nextRotationBlock,  // l2BlockNumber (at rotation boundary: 100, 200, 300...)
          stateRoot,
          transactionsRoot
        );
      });
      
      logBatch(nextRotationBlock, 'Transaction sent', { hash: tx.hash });
      ...
      if (receipt && receipt.status === 1) {
        ...
        await markBatchSubmitted(batchNumber, tx.hash);
        
        logBatch(batchNumber, 'Submitted successfully', {
          l1TxHash: tx.hash,
   ...
   * Collect block data from L2
   */
  private async collectBlockData(
    startBlock: bigint,
    endBlock: bigint
  ): Promise<{ stateRoot: string; transactionsRoot: string }> {
    try {
