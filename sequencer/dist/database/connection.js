...
async function initDatabase(databaseUrl) {
    ...
    try {
        ...
        throw new types_1.SequencerError('Database connection failed', types_1.ErrorCode.DATABASE_ERROR, error);
    }
}
/**
 * Get database pool
...
function getPool() {
    if (!pool) {
        throw new types_1.SequencerError('Database not initialized', types_1.ErrorCode.DATABASE_ERROR);
    }
    return pool;
}
/**
...
async function query(text, params) {
    ...
    try {
        ...
        logger_1.logger.error('Database query failed', { text, error });
        throw error;
    }
}
/**
...
async function runMigrations() {
    ...
    for (const migration of migrations) {
        try {
            ...
            logger_1.logger.error('Migration failed', { migration, error });
            throw error;
        }
    }
